<template>
  <el-row type="flex"  align="middle">
    ...
    <el-col :span="6" class="avatar" :offset="4">
      ...
      <ul
        class="show-controls"
        v-if="userInfo.token && isSpread"
        @click.capture="selectItems"
      >
        <li>个人信息</li>
        <li>退出登录</li>
      </ul>
...
export default {
  ...
  methods: {
    ...
    async selectItems(e) {
      ...
      if (detail === '退出登录') {
        const flag = await this.$store.dispatch('loginModule/userLogout')
        console.log(flag)
        if (flag) {
          this.$message({
            type: 'success',
...
.el-row {
  ...
  .el-col {
    ...
    // logo 图标
    &.home-logo {
      .imgPosition('~@/static/tabbar.png',0,-5px);
    }
    ...
    &.avatar {
      ...
      ul.show-controls {
        ...
        list-style: none;
        padding-top: 5px;
        box-shadow: 2px 2px 5px @shadowColor, 2px -2px 5px @shadowColor,
          -2px 2px 5px @shadowColor, -2px -2px 5px @shadowColor;
        border-radius: 3px;
        &:before {
          content: '';
          position: absolute;
